...
{        
        ...
        options.c_cc[VMIN] = 1; /*минимальное число байт для чтения*/
        ...
        options.c_oflag &= ~OPOST;  /*Обязательно отключить постобработку*/
         ...
         //
         options.c_iflag &= ~(IGNBRK | BRKINT | ICRNL | INLCR | PARMRK | INPCK | ISTRIP );
...
int readData(int F_ID, unsigned char *buff, int size, int timeout_value)
{
        ...
        if (F_ID > 0)
        {
            ...
            if (n == -1)
            {
                ...
                syslog(LOG_WARNING | LOG_PID, "UART Read error");                
            ...
            {                
				//total = read(F_ID, buff, size); /* there was data to read */
                while (total < size)
                {
...
{    
		tcflush(F_ID, TCOFLUSH | TCIFLUSH);
